...
namespace CR::Engine::Core {
	...
	// This is basically a std::bitset, except it can iterator 1 bits fast, and find the first 0 bit
	// fast. The API is different that std::bitset, its treated more as a set of integers. The API
	// follows std::set instead. The Size template parameter represents the largest integer that the
	// set can contain + 1. i.e. Size 512 means this set can hold integers from 0 to 511. Size must be
	// a multiple of 64. Multiples of 512 may make more sense in practice, since you can fit 512 bits
	// in a cache line.
	//
	// The current implementation is the fastest for sets with Size <= 2048 or so. Could use an
	// alternate implementation for Sizes 2049-16384; Basically needs to burn one cache line for some
	// simple acceleration data, current size(), min/max contained value, ect. And maybe switch the
	// bitset itself to a heap allocation. Larger than 16384 probably faster to use something like
	// roaring bitmap. With that in mind, this class has a hard cap at 64K.
	...
	public:
		...
		[[nodiscard]] constexpr uint16_t size() const noexcept {
			...
			CR_ASSERT_AUDIT(result <= std::numeric_limits<uint16_t>::max(),
		...
		public:
			...
			bool operator==(const ConstIterator& a_other) const {
				CR_ASSERT_AUDIT(&m_bitset == &a_other.m_bitset,
				                "comparing iterators from different bitsets");
