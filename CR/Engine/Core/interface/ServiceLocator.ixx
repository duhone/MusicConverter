	...
	public:
		...
		T& Add(ArgsT&&... args) {
			...
			std::unique_ptr<Service> service =
			    std::make_unique<ServiceImpl<T>>(std::forward<ArgsT>(args)...);
		...
		T& Get() {
			...
			CR_ASSERT(serviceIter != m_services.end(), "Could not find service {}",
			          EightCC(T::s_typeIndex));
			CR_ASSERT_AUDIT(serviceIter->second.get(), "Service {} not constructed",
			                EightCC(T::s_typeIndex));
