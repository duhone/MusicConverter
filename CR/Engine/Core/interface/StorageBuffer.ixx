...
namespace CR::Engine::Core {
	...
	// Still following standard proposal though just in case, if it doesn make it in, easy to swap
	// this out. This is slightly simplified from standard proposal. Doesn't support string/vector
	// interop. Also doesn't support pmr allocators at this time.
	...
	inline CR::Engine::Core::StorageBuffer<T, Allocator>::StorageBuffer(
	    const Allocator& alloc) noexcept : m_allocator(alloc) {}
	...
	inline CR::Engine::Core::StorageBuffer<T, Allocator>&
	    CR::Engine::Core::StorageBuffer<T, Allocator>::operator=(
	        StorageBuffer<T, Allocator>&& s) noexcept {
	...
	template<typename T, typename Allocator>
	inline typename StorageBuffer<T, Allocator>::iterator
	    StorageBuffer<T, Allocator>::begin() noexcept {
		return m_data;
	}
	...
	template<typename T, typename Allocator>
	inline typename StorageBuffer<T, Allocator>::iterator
	    StorageBuffer<T, Allocator>::end() noexcept {
	...
	template<typename T, typename Allocator>
	inline typename StorageBuffer<T, Allocator>::const_reference
	    StorageBuffer<T, Allocator>::front() const {
		CR_ASSERT(m_size > 0, "tried to get front element on an empty buffer");
		return m_data[0];
	}
	...
	template<typename T, typename Allocator>
	inline typename StorageBuffer<T, Allocator>::const_reference
	    StorageBuffer<T, Allocator>::back() const {
		CR_ASSERT(m_size > 0, "tried to get back element on an empty buffer");
		return m_data[m_size - 1];
	}

	template<typename T, typename Allocator>
	inline typename StorageBuffer<T, Allocator>::pointer
	    StorageBuffer<T, Allocator>::data() noexcept {
		return m_data;
	}
