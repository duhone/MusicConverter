...
export namespace CR::Engine::Core {
	...
	// t_column... is a struct(or primitive) holding the data for one column of a row. t_column...
	// should not contain the primary key. You can have more than one t_column type. This allows
	// creating a SOAOS layout. The primary key is in one array, and each column in an array. You can
	// iterate the rows of the table for just one column. You can grab multiple iterators if you need
	// to walk multiple columns at once. The fastest way to get to a row for a column is by index, not
	// by primary key, you can save these indices for future use. Indices are stable.
	//
	// c_maxSize - maximum rows. this many rows is allocated up front currently. Can use a lot of
	// memory There can be no more than 64K-1 rows. If you need more than that, probably should be
	// writing custom code. Using arrays currently, and std::bitset. For larger tables need a bitset
	// replacement(which would help here too). And should use std::vector instead of arrays for larger
	// tables.
	...
	class Table {
		...
		// boyce-codd normal form. Or you just need to split up the table. You can of course have
		// multiple columns.
	...
	public:
		...
		// problematic when t_primaryKey is a string type(char*, string, string_view, ect) should just
		// take a string_view for those. Wait until C++20 to fix, can be done cleaner there.
		...
		public:
			...
			ConstIterator<t_viewSubset...> begin() const {
				return ConstIterator<t_viewSubset...>(&this, 0);
			}
			ConstIterator<t_viewSubset...> end() const {
				return ConstIterator<t_viewSubset...>(&this, c_maxSize);
			}
			ConstIterator<t_viewSubset...> cbegin() const {
				return ConstIterator<t_viewSubset...>(&this, 0);
			}
...
void CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::ClearColumnDefault(
	...
	if constexpr(!std::is_standard_layout_v<t_column>) {
		std::get<t_column>(m_rows)[a_index] = t_column{};
	}
}
...
template<uint16_t c_maxSize, std::regular t_primaryKey, typename... t_columns>
void CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::ClearRowDefault(
    uint16_t a_index) {
...
template<typename t_primaryKeyF, typename... t_columnsF>
inline uint16_t
    CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::insert(t_primaryKeyF&& a_key,
                                                                           t_columnsF&&... a_row)
  requires !std::same_as<t_primaryKey, std::string>
{
	uint16_t unusedIndex = insert(std::forward<t_primaryKeyF>(a_key));

	ClearRow(unusedIndex, std::index_sequence_for<t_columnsF...>{},
	         std::forward<t_columnsF>(a_row)...);
...
inline uint16_t CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::insert(
    const std::string_view a_key, t_columnsF&&... a_row)
  requires std::same_as<t_primaryKey, std::string>
{
...
template<uint16_t c_maxSize, std::regular t_primaryKey, typename... t_columns>
template<typename t_value>
inline const t_value& CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::GetValue(
    uint16_t a_index) const {
...
template<typename t_value>
inline t_value&
    CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::GetValue(uint16_t a_index) {
	CR_REQUIRES_AUDIT(a_index != c_unused && m_used[a_index], "asked for an unused row in table {}",
	                  m_tableName);
...
[[nodiscard]] std::tuple<t_columns&...>
    CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::GetValues(
        uint16_t a_index, std::index_sequence<I...>) {
	return std::tuple<t_columns&...>(std::get<I>(m_rows)[a_index]...);
}

template<uint16_t c_maxSize, std::regular t_primaryKey, typename... t_columns>
[[nodiscard]] const std::tuple<const t_columns&...>
    CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::operator[](
        uint16_t a_index) const {
	CR_REQUIRES_AUDIT(a_index != c_unused && m_used[a_index], "asked for an unused row in table {}",
	                  m_tableName);
...
template<uint16_t c_maxSize, std::regular t_primaryKey, typename... t_columns>
inline void
    CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::erase(uint16_t a_index) {
	CR_REQUIRES_AUDIT(a_index != c_unused && m_used[a_index],
	                  "tried to delete an unused row in table {}", m_tableName);

	m_used[a_index] = false;

	// To save on memory, if not a standard layout, then clear out the data. Hopefully if not standard
	// layout then a proper move assignment was written.
	if constexpr(!std::is_standard_layout_v<t_primaryKey>) {
		m_primaryKeys[a_index] = t_primaryKey{};
	}
...
CR::Engine::Core::Table<c_maxSize, t_primaryKey,
                        t_columns...>::Iterator<t_columnsSubset...>::Iterator(Table& a_table,
                                                                              uint16_t a_index) :
...
template<uint16_t c_maxSize, std::regular t_primaryKey, typename... t_columns>
template<typename... t_columnsSubset>
auto CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::Iterator<
    t_columnsSubset...>::operator++() -> Iterator& {
...
template<typename... t_columnsSubset>
auto CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::Iterator<
    t_columnsSubset...>::operator++(int) -> Iterator& {
	Iterator tmp = *this;
	++(*this);
	return tmp;
}
...
auto CR::Engine::Core::Table<c_maxSize, t_primaryKey,
                             t_columns...>::Iterator<t_columnsSubset...>::operator*() -> reference {
	return m_binding;
}
...
template<uint16_t c_maxSize, std::regular t_primaryKey, typename... t_columns>
template<typename... t_columnsSubset>
CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::ConstIterator<
    t_columnsSubset...>::ConstIterator(const Table& a_table, uint16_t a_index) :
    m_table(a_table), m_index(a_index) {
	while(m_index < c_maxSize && !m_table.m_used[m_index]) { ++m_index; }
	SetBinding();
}
...
template<uint16_t c_maxSize, std::regular t_primaryKey, typename... t_columns>
template<typename... t_columnsSubset>
auto CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::ConstIterator<
    t_columnsSubset...>::operator++() -> ConstIterator& {
	if(m_index == c_maxSize) { return *this; }
	++m_index;
	while(m_index < c_maxSize && !m_table.m_used[m_index]) { ++m_index; }
	SetBinding();
...
template<typename... t_columnsSubset>
auto CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::ConstIterator<
    t_columnsSubset...>::operator++(int) -> ConstIterator& {
	Iterator tmp = *this;
	++(*this);
	return tmp;
}
...
template<typename... t_columnsSubset>
auto CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::ConstIterator<
    t_columnsSubset...>::operator*() -> reference {
	return m_binding;
}
...
template<typename... t_columnsSubset>
auto CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::ConstIterator<
    t_columnsSubset...>::operator->() -> pointer {
	return &m_binding;
}
